...
public class TestAlgorithms extends NeuralNetwork
{
    ...
    public void run()
    {
        ...
        for (int i = 1; i <= operators; i++) // for each initialisation mode
        {
            ...
            {
                System.out.println("\n\n--i: " + j + " out of " + times + "\n");
                System.out.println("Testing for initialisation " + replacement[i - 1]);
                String key_type = Integer.toString(j)+" "+replacement[i-1];
                best_ones.put(key_type, ea.runAlgorithm(initialisation[0], selection[2], crossover[1], mutation[2], diversity[1], replacement[i-1]).toString());
            }
        }
        ...
        try(FileWriter fw = new FileWriter("results/replacement.txt", true);
